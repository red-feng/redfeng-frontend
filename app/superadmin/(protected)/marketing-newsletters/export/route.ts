...
export async function GET(request: Request) {
  ...
  if (query) subscriberQuery = subscriberQuery.ilike("email", `%${query}%`)
  if (status !== "all") subscriberQuery = subscriberQuery.eq("status", status)
  if (source !== "all") subscriberQuery = subscriberQuery.eq("source_path", source)
