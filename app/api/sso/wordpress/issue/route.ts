...
export async function GET(request: Request) {
  try {
    ...
    if (!profile) {
      ...
      if (upsertError) {
        return NextResponse.json(
          { error: `Gagal menyiapkan profile customer: ${upsertError.message}` },
          { status: 500 },
        )
      }
    ...
    if (error) {
      return NextResponse.json(
        { error: `Gagal membuat token SSO WordPress: ${error.message}` },
        { status: 500 },
      )
    }

    const redirectUrl = buildWordPressSsoRedirectUrl(rawToken, redirectPath)
    return NextResponse.redirect(redirectUrl)
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error"
    return NextResponse.json({ error: `SSO WordPress issue failed: ${message}` }, { status: 500 })
  }
}
