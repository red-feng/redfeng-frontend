      ...
      <div className="mx-auto max-w-4xl overflow-hidden rounded-[36px] border border-orange-200/60 bg-white shadow-[0_28px_90px_rgba(146,64,14,0.12)]">
        <section className="bg-[linear-gradient(135deg,#7f1d1d_0%,#dc2626_40%,#fb923c_100%)] px-8 py-10 text-white sm:px-10">
          ...
          <h1 className="mt-5 text-4xl font-semibold tracking-tight sm:text-5xl">
            {isExpired ? "Batas revisi merchant sudah berakhir" : "Merchant perlu revisi dari operations manager"}
          </h1>
          <p className="mt-4 max-w-2xl text-base leading-8 text-orange-50/90">
            {isExpired
              ...
              : "Operations manager meminta perbaikan data sebelum merchant dapat masuk ke dashboard partner Red Feng."}
          </p>
        </section>
        ...
        <section className="grid gap-6 bg-[linear-gradient(180deg,#fffdfa_0%,#fff8f1_100%)] px-8 py-10 sm:px-10 lg:grid-cols-[1.2fr_0.8fr]">
          <article className="rounded-[28px] border border-red-200 bg-white p-6 shadow-[0_20px_50px_rgba(127,29,29,0.08)]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">Review result</p>
                <p className="mt-1 text-2xl font-semibold text-slate-950">
                  {isExpired ? "Revisi melewati tenggat" : "Revisi data diperlukan"}
                </p>
              </div>
            </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <div className="rounded-[20px] border border-slate-200 bg-slate-50/80 px-5 py-4">
                ...
                <p className="mt-2 text-sm font-semibold text-slate-900">{isExpired ? "Expired" : "Revision Requested"}</p>
              </div>
            ...
            <p className="mt-5 text-sm font-semibold text-slate-900">Alasan operations manager</p>
          ...
          <aside className="rounded-[28px] border border-slate-200 bg-slate-50/80 p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">What to do next</p>
            {!isExpired ? (
              <ul className="mt-4 space-y-3 text-sm leading-7 text-slate-600">
                <li>Tinjau alasan revisi dari operations manager.</li>
                ...
                <li>Jika lewat 7 hari tidak ada revisi, akun merchant akan masuk flow penghapusan permanen.</li>
              </ul>
            ) : (
              <ul className="mt-4 space-y-3 text-sm leading-7 text-slate-600">
