...
export default function HeroBenefits({ activeTab }: HeroBenefitsProps) {
  ...
  return (
    <div className="mt-6 grid grid-cols-2 gap-y-6 border-t border-[#edf1f5] px-1 pt-7 text-sm text-slate-600 sm:grid-cols-4 lg:mt-6 lg:gap-y-0 lg:pt-8">
      {benefits.map((item) => {
        const Icon = item.icon
        return (
          <div key={item.title} className="flex flex-col items-center gap-3 text-center">
            <div className="flex h-[54px] w-[54px] items-center justify-center rounded-full bg-white text-slate-700 ring-1 ring-[#d8e1eb]">
