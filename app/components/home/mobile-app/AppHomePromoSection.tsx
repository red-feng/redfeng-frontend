...
export default function AppHomePromoSection({ locale, promos }: { locale: Locale; promos: MarketingPromo[] }) {
  const copy = {
    id: {
      eyebrow: "Promo aktif",
      ...
      <div className="mt-4 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {promos.map((card, index) => (
          ...
          >
            ...
            <div className={`absolute inset-0 bg-no-repeat ${card.imageClass}`} style={{ backgroundImage: `url('${card.image}')` }} />
