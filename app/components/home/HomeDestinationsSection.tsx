      ...
      <section className="mx-auto max-w-[1240px] px-4 pb-6 sm:px-6 lg:px-8">
        <div className="flex gap-4 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:grid-cols-2 sm:overflow-visible lg:grid-cols-3 xl:grid-cols-6">
          {destinations.map((destination) => (
            <article key={destination.name} className="group relative h-[150px] w-[106px] min-w-[106px] overflow-hidden rounded-[18px] shadow-[0_18px_34px_-28px_rgba(15,23,42,0.3)] sm:h-[160px] sm:w-auto sm:min-w-0">
              ...
              <div className="absolute inset-x-4 bottom-4 text-white">
                <h3 className="text-[13px] font-bold leading-none tracking-[-0.04em] sm:text-[24px]">{destination.name}</h3>
