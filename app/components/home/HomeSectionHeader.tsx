...
export default function HomeSectionHeader({ title, showTabs = false }: HomeSectionHeaderProps) {
  return (
    <section className={`home-section-header mx-auto max-w-[1240px] px-4 pb-4 ${showTabs ? "pt-9" : "pt-6"} sm:px-6 lg:px-8`}>
      <div className="home-section-header-shell flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="home-section-header-tabs flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:flex-wrap">
              {bookingTabs.map((tab, index) => (
                <button
                  key={tab}
                  className={`home-section-header-tab shrink-0 rounded-full border px-4 py-2 text-[13px] font-medium transition ${
