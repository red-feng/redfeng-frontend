...
const services = [
  { label: "Hotel", tone: "bg-[#215ea8]", Icon: BuildingIcon },
  { label: "Tiket Pesawat", tone: "bg-[#34c4f4]", Icon: PlaneIcon },
  { label: "Aktivitas", tone: "bg-[#ff6b74]", Icon: SparklesIcon },
  { label: "Tiket Kereta", tone: "bg-[#ffb100]", Icon: TrainIcon },
  { label: "Tiket Bus", tone: "bg-[#2dc84f]", Icon: BusIcon },
  { label: "Kapal", tone: "bg-[#ff8a8a]", Icon: ShipMiniIcon },
  { label: "Pesawat + Hotel", tone: "bg-[#9129c1]", Icon: FlightHotelIcon },
  { label: "Transfer", tone: "bg-[#19b6b0]", Icon: TransferIcon },
  { label: "Paket Wisata", tone: "bg-[#9f1f43]", Icon: PackageMiniIcon },
...
export default function HomeStandaloneMobileHero() {
  return (
    <section className="standalone-home-top relative md:hidden">
      <div className="rounded-b-[36px] bg-[linear-gradient(180deg,#19a6f4_0%,#0f97eb_62%,#0b8de4_100%)] px-4 pb-20 pt-[calc(env(safe-area-inset-top)+0.85rem)] shadow-[0_18px_42px_-30px_rgba(14,149,234,0.65)]">
        <div className="flex items-center gap-3">
          <button
            type="button"
            className="flex h-14 flex-1 items-center gap-3 rounded-full bg-white px-5 text-left shadow-[0_14px_30px_-24px_rgba(15,23,42,0.32)]"
          >
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {quickChips.map((chip, index) => (
            <button
              key={chip}
              type="button"
              className={`shrink-0 rounded-full px-4 py-2 text-[12px] font-semibold shadow-[0_14px_24px_-22px_rgba(15,23,42,0.32)] ${
                index === 0 ? "bg-[#0f7dce] text-white" : "bg-[#119df0] text-white/95"
              }`}
            >
              {chip}
            </button>
