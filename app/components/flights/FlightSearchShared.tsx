...
export function FlightSearchTripTab({
  active,
  label,
  onClick,
  tone = "default",
  ...
  const inactiveClass =
    tone === "inverse"
      ? "border-transparent bg-transparent text-white/88 hover:bg-white/10 hover:text-white"
...
export function FlightSearchStaticField({
  icon,
  label,
  value,
  sublabel,
...
export function FlightSearchInteractiveField({
  icon,
  label,
  withChevron = false,
  children,
  ...
  return (
    <div className="flex min-w-0 items-center gap-3 rounded-[22px] border border-[#dfe7f1] bg-white px-3.5 py-3 shadow-[0_10px_24px_-20px_rgba(15,23,42,0.2)] md:px-4.5">
      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#fff3ee] text-[#ef4423] shadow-inner">
        {icon}
      </span>
