...
import BusHeroSearchBar from "@/app/components/bus/BusHeroSearchBar"
import { getMarketingPromosResolved } from "@/lib/marketing-content"
import { getCurrentLocale } from "@/lib/locale"

type BusMarketingLandingProps = {
...
export default async function BusMarketingLanding({ searchParams }: BusMarketingLandingProps) {
  ...
  const baseCopy = {
    id: {
      ...
      benefitBody: [
        ...
        "Arah CTA dan ritme kartu tetap satu keluarga dengan landing page paket.",
        "Bus reguler dan sleeper bisa ditampung dalam bahasa visual yang sama.",
        "Saat operator live siap, halaman ini tinggal disambungkan ke hasil real-time.",
      ],
      promoTitle: "Promo Rute Bus",
    ...
    en: {
      ...
      benefitBody: [
        ...
        "CTA direction and card rhythm stay in the same family as the package landing page.",
        "Regular buses and sleeper trips fit inside the same visual language.",
        "When live operators are ready, this page can connect directly to real-time results.",
      ],
      promoTitle: "Bus Route Deals",
    ...
    zh: {
      ...
      promoBody: "最高可享",
      promoAction: "查看优惠",
      offerLine: "覆盖客户最常搜索的热门陆路路线与卧铺线路",
      popularTitle: "热门路线",
      seeAll: "查看全部路线",
      fromLabel: "起价",
  ...
  const routes = [
    ...
    { title: "Jakarta - Malang", price: "IDR 410.000", image: "/home-assets/card-package.png" },
    { title: "Bandung - Surabaya", price: "IDR 360.000", image: "/home-assets/card-package.png" },
    { title: "Jakarta - Solo", price: "IDR 340.000", image: "/home-assets/card-package.png" },
    { title: "Semarang - Denpasar", price: "IDR 470.000", image: "/home-assets/card-package.png" },
    { title: "Yogyakarta - Banyuwangi", price: "IDR 290.000", image: "/home-assets/card-package.png" },
  ]
      ...
      <main className={`${homeLayoutLock.pageXClass} pb-14 pt-2 md:pt-3`}>
        ...
        <section className={`${homeLayoutLock.contentWidthClass} relative z-[210] -mt-20 sm:-mt-24 lg:-mt-28`}>
          <div className={`${homeLayoutLock.wideContentWidthClass} ${homeLayoutLock.cardRadiusClass} border border-[#f0e4da] bg-white p-3 shadow-[0_28px_56px_-26px_rgba(15,23,42,0.22)] md:p-4`}>
            <BusHeroSearchBar locale={locale} buttonLabel={copy.searchButton} />
          </div>
              ...
              <article key={title} className="rounded-[24px] border border-white/85 bg-white/78 p-4 shadow-[0_22px_40px_-30px_rgba(15,23,42,0.18)] backdrop-blur sm:rounded-[26px] sm:p-5">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#fff3ee] text-[#ef4423]">
                  <svg viewBox="0 0 24 24" className="h-6 w-6 fill-none stroke-current stroke-[1.8]">
                    {index === 0 ? <rect x="5" y="4.5" width="14" height="12" rx="2.5" /> : index === 1 ? <path d="M7 12l3 3 7-7" /> : index === 2 ? <path d="M8 16.5 6.5 19M16 16.5l1.5 2.5" /> : <circle cx="11" cy="11" r="6.5" />}
                    {index === 0 ? <path d="M8 16.5 6.5 19M16 16.5l1.5 2.5M8 9h.01M16 9h.01M5 12.5h14" /> : index === 1 ? <rect x="4" y="6" width="16" height="12" rx="2.5" /> : index === 2 ? <path d="M5 12.5h14" /> : <path d="M16 16l4 4" />}
                  </svg>
                </div>
        ...
        <section id="bus-promo" className={`${homeLayoutLock.contentWidthClass} ${homeLayoutLock.cardRadiusSmClass} mt-10 rounded-[28px] border border-[#efe2d8] bg-[linear-gradient(180deg,#fff7f3_0%,#fffdfa_100%)] p-4 shadow-[0_26px_70px_-44px_rgba(15,23,42,0.18)] sm:p-6`}>
          <PromoPlacementImpressionBeacon placement={placementUsed || "homepage_feed"} sourcePath="/bus" promos={promos.map((promo) => ({ id: promo.id, slug: promo.slug }))} />
          <div className="grid gap-5 xl:grid-cols-[300px_repeat(4,minmax(0,1fr))]">
            <article className="rounded-[28px] bg-[linear-gradient(180deg,#fff5f2_0%,#fffaf8_100%)] p-5">
              ...
              <p className="mt-2 text-[17px] font-bold leading-none tracking-[-0.03em] text-[#ef4423] sm:text-[19px]">
                18% <span className="text-[13px] sm:text-[14px]">OFF</span>
              </p>
