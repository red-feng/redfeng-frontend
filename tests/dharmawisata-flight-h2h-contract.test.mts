...
for (const anchor of [
  ...
  "schReturns: []",
    ...
    "function summarizeBookingResponse(raw: JsonRecord)",
...
for (const anchor of [
  ...
  "`Airline/PriceAllAirline` is called with `POST /Airline/PriceAllAirline`.",
  "It must use the all-airline price payload: `airlineID`, `origin`, `destination`, `tripType`, `departDate`, `returnDate`, `paxAdult`, `paxChild`, `paxInfant`, `airlineAccessCode`, `journeyDepartReference`, `journeyReturnReference`, `userID`, and the same `accessToken`.",
  "`Airline/PriceAllAirline` responses may include `airlineAccessCode`, `searchKey`, `promoCode`, `priceDepart`, `priceReturn`, `sumFare`, `respTime`, `userID`, `accessToken`, `status`, and `respMessage`.",
  ...
  "`Airline/BaggageAndMeal` is called with `POST /Airline/BaggageAndMeal`.",
  ...
  "`Airline/BaggageAndMeal` responses may include `addOns`, `respTime`, `userID`, `accessToken`, `status`, and `respMessage`.",
  "including add-on count and non-PII request summary",
  "`Airline/Seat` is called with `POST /Airline/Seat`.",
  "Its request body is `application/json` with `airlineID`, `origin`, `destination`, `tripType`, `departDate`, `returnDate`, `schDepart`, `schReturn`, `paxAdult`, `paxChild`, `paxInfant`, `departureAirlineSegmentCode`, `departureFareBasisCode`, `returnAirlineSegmentCode`, `returnFareBasisCode`, passenger contact fields, `paxDetails`, `insurance`, `userID`, and the same `accessToken`.",
  ...
  "including seat add-on count and non-PII request summary",
    ...
    "`Airline/Booking` is the mutating hold endpoint and is called with `POST /Airline/Booking`.",
    "`Airline/Booking` returns supplier identifiers and hold metadata",
    "`airlineAdminFee`, `memberAdminFee`, `memberDiscount`, `salesPrice`, `ticketPrice`, and `currency`",
