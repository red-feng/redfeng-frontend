...
const INTERNAL_CHAT_ATTACHMENT_BUCKET = "internal-chat-attachments"
const MAX_INTERNAL_CHAT_ATTACHMENT_BYTES = 10 * 1024 * 1024
...
export function isInternalChatImageAttachment(mimeType: string | null | undefined) {
  return String(mimeType || "").toLowerCase().startsWith("image/")
}

export async function uploadInternalChatAttachment(input: {
  roomId: string
  senderId: string
  file: File | null
}) {
  ...
  const { error: uploadError } = await adminSupabase.storage
    .from(INTERNAL_CHAT_ATTACHMENT_BUCKET)
  ...
  const { data } = adminSupabase.storage.from(INTERNAL_CHAT_ATTACHMENT_BUCKET).getPublicUrl(objectPath)
